...
Configuration discovery starts at the runner working directory and walks upward until it finds the first `loglensreporter.config.json`. When no file exists, adapters use their built-in defaults. Invalid JSON, unknown keys, and invalid values fail with the complete config path and option name.
...
The formatter reads the width from `stdout`, `stderr`, or `COLUMNS`. Automatically detected widths are capped at `120` columns so output remains readable on wide terminals. A numeric `terminalWidth` bypasses that cap. The formatter reserves four columns by default, including when `terminalWidth` is explicit, because terminal wrappers and auto-wrap behavior may consume part of the reported width. Set `terminalMargin: 0` to disable the reserve or increase it when another process prefixes every output line.

When no width is available, the formatter uses an `80`-column fallback and renders at most `76` visible columns. The header, progress body, and summary are connected sections with the same outer width. Neighboring sections share one border, so no empty rows or duplicate border rows separate them. The summary places result, duration, and all counters on one row when they fit. A dotted divider separates the title from the values, and complete value segments wrap on narrower terminals without being clipped. Progress rows and diagnostics are padded to the shared right border. Test paths use all available path-column space by default. When a path does not fit, parent directories are shortened from left to right by only the required number of characters; the filename is shortened only after every parent directory reaches one character. An explicit `pathWidth` limits the column. The same layout and path algorithms are shared by every adapter. Long diagnostics wrap inside the body frame instead of relying on terminal auto-wrap.

The active width is captured when a run header is printed and reused for its body, diagnostics, and summary. Resizing the terminal during a streaming test run therefore cannot split the right border across different columns.

Divider weight follows a visual hierarchy. Double Unicode borders use single solid section dividers and a dashed summary divider. Single Unicode borders use dashed section dividers and a finer middle-dot summary divider.

Runner headers limit title and metadata text to `48` visible columns. Metadata is generated as `Provider · base/path · N files · N tests`. Adapters infer the shared base path from every collected test file, including every category in a mixed Playwright run. Set `showDescription: false` to render only the title row.

Progress rows reserve stable columns for the test path, test count, duration, and final status. Directory segments are shortened when needed, while `PASSED`, `FAILED`, `FLAKY`, and the other outcomes remain aligned. Shortened basenames use the ASCII marker `...` because Unicode ellipsis has ambiguous terminal width. On narrow terminals, the formatter first removes alignment padding and then shortens the test-count label before clipping any remaining content, so the right border always stays inside the active width.
...
Adapters write progress and its nested test diagnostics to `log`, preserving their order in redirected output. Standalone process, build, and global diagnostics are written to `error`:
